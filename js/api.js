...
export async function testSupabaseConnection() {
  const supabase = getSupabaseClient();
  const { data, error } = await supabase.from('classes').select('id, name').limit(1);
  if (error) throw error;
  return data;
}
...
export async function closeSession(sessionId) {
  const supabase = getSupabaseClient();
  const { error } = await supabase.from('test_sessions').update({ is_open: false }).eq('id', sessionId);
  if (error) throw error;
}
...
export async function importStudents(classId, displayNames) {
  const supabase = getSupabaseClient();
  const rowsToInsert = displayNames.map(full_name => ({ class_id: classId, full_name }));
...
export async function loadSubmittedStudentIds(sessionId) {
  const supabase = getSupabaseClient();
  const { data, error } = await supabase.from('submissions').select('student_id').eq('session_id', sessionId);
  if (error) throw error;
...
export async function loadResultsDetailed() {
  ...
  const [{ data: classes, error: classesError }, { data: tests, error: testsError }] = await Promise.all([
    ...
    testIds.length ? supabase.from('tests').select('id, title').in('id', testIds) : Promise.resolve({ data: [], error: null })
  ]);
...
export async function resetSubmission(submissionId) {
  const supabase = getSupabaseClient();
  const { error: answersError } = await supabase.from('submission_answers').delete().eq('submission_id', submissionId);
  if (answersError) throw answersError;

  const { error: submissionError } = await supabase.from('submissions').delete().eq('id', submissionId);
  if (submissionError) throw submissionError;
}
